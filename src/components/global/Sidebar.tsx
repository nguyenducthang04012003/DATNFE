...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const canAccessMenu = (menuKey: string) => {
    ...
    switch (menuKey) {
      ...
      case "Nhà thuốc":
        return (
          role === "SalesManager" || role === "Director" || role === "SalesMan"
        );
      case "Đơn đặt hàng":
    ...
    switch (menuKey) {
      ...
      case "Đơn đặt hàng":
        return items.filter((item) =>
          item === "Danh sách đơn đặt hàng(PO)"
            ? role === "WarehouseManager" || role === "Director"
            : role === "Director"
        );
      case "Phiếu nhập kho":
        return items.filter((item) =>
      ...
      case "Kho":
        return items.filter((item) =>
          item === "Tạo kho mới" ? role === "Director" : true
        );
      case "Đơn hàng":
      ...
      case "Phiếu kiểm kê":
        return items.filter((item) =>
          item === "Tạo phiếu kiểm kê" ? role === "WarehouseManager" : true
        );
      default:
        return items;
    }
  ...
  return (
    <div className="w-[260px] min-w-[260px] max-w-[260px] flex-shrink-0 border-r-[1px] border-r-gray-200 z-20 bg-white h-full overflow-y-auto">
      <div className="w-full">
        <img
          className="w-[150px] mx-auto my-1"
          src="/img/logoPharma.png"
          alt="Pharma Logo"
        />
      </div>
            ...
            <div key={menuKey}>
              <div
                className={`px-[15px] flex justify-between items-center cursor-pointer rounded-[4px] transition-all py-2.5 text-[15px] 
                  ${
                    openMenu === menuKey
                      ? "text-white bg-[#1b2850]"
                      : "hover:text-white hover:bg-[#1b2850]"
                  }`}
              ...
              >
                <span className="flex items-center">
                  ...
                  {menuKey === "Nhà cung cấp" && (
                    <StoreIcon className="mr-[6px] w-4 h-4" />
                  )}
                  {menuKey === "Đơn đặt hàng" && (
                  ...
                  {menuKey === "Phiếu nhập kho" && (
                    <ArrowRightEndOnRectangleIcon className="mr-[6px] w-4 h-4" />
                  )}
                  {menuKey === "Phiếu xuất kho" && (
                    <ArrowRightStartOnRectangleIcon className="mr-[6px] w-4 h-4" />
                  )}
                  {menuKey === "Kho" && (
                    <StoreIcon className="mr-[6px] w-4 h-4" />
                  )}
                  {menuKey === "Phiếu kiểm kê" && (
                    <ClipboardDocumentCheckIcon className="mr-[6px] w-4 h-4" />
                  )}
                  {menuKey.charAt(0).toUpperCase() + menuKey.slice(1)}
                </span>
                <ChevronRightIcon
                  className={`w-4 h-4 transition-transform ${
                    openMenu === menuKey ? "rotate-90" : ""
                  }`}
                />
              </div>
              <ul
                className={`transition-all duration-300 ease-in-out overflow-hidden ${
                  openMenu === menuKey ? "max-h-[500px]" : "max-h-0"
                }`}
              >
